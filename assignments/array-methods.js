...
let ticketPriceTotal = runners.reduce(function(add, currentValue){
  return add + currentValue.donation;
}, 0);
...
// Problem 1 - .forEach()
// The community center needs the emails and company names of the runners to add them their email list for next year's run
let emailList = [];
runners.forEach(function(currentValue){
  return emailList.push(`${currentValue.email}, ${currentValue.company_name}`);
});
console.log(emailList);

// Problem 2 - .filter()
//The community center wants to send gift baskets to every business who has donated more than 200 hundred dollars
let bigDonors = [];
runners.filter(function(currentValue){
  if ( currentValue.donation > 200){
    return bigDonors.push(`${currentValue.company_name}`);
  };
});
console.log(bigDonors);

// Problem 3 - .map()
// Console log just the business IDs

let businessId = runners.map(function(currentValue){
